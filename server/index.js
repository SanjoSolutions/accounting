...
app.put('/settings', async function (request, response) {
    ...
    const accountId = '1';
    const account = (await accounts.findOne(accountId));
    Object.assign(account.invoiceIssuer, pick(request.body.invoiceIssuer, ['name', 'streetAndHouseNumber', 'zipCode', 'city', 'country']));
...
app.post('/documents/:id/parsing-requests', async function (request, response) {
    ...
    if (document && document.gsURL) {
        ...
        invoice.tax = new TaxAmount(getTaxAmount(data), new Tax('19% VAT', 0.19));
...
async function downloadResultWithFixture() {
    return await readJSON(resolve(__dirname, 'dataFixtures/results_11048337544652359545_0_Invoice_Example_English-0.json'));
}
function getNetAmount(data) {
...
function getEntity(data, type) {
    return getObjectWithType(data.entities, type);
}
function getProperty(entity, type) {
    return getObjectWithType(entity.properties, type);
}
function getObjectWithType(array, type) {
    return array.find((object) => object.type === type) ?? null;
}
function getMoneyValue(entity) {
