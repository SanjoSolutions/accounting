...
export function DocumentUpload({ onDocumentUploaded }: { onDocumentUploaded: (document: Document) => void}) {
  ...
  const onSubmit = useCallback(
    (event: React.FormEvent<HTMLFormElement>) => {
      ...
      //       const { data: document } = await getJSON(await api.post('/api/documents', {
      //         url: downloadURL,
      //         gsURL: ref.toString(),
      //       }))
      //       const { data: invoice } = await getJSON(await api.post(
      //         `/api/documents/${ document.id }/parsing-requests`,
      //         {},
      //       ))
