...
describe('tax workflow integration migration', () => {
  it('creates tenant-scoped durable tax, VAT and structured-invoice storage', () => {
    ...
    expect(tables).toEqual(expect.arrayContaining(['StructuredInvoice', 'InvoiceNumberReservation', 'InvoiceNumberSequenceOnboarding', 'VatPostingRecord', 'VatReversalMarker', 'TaxWorkflowRecord', 'TaxSubmissionRequest', 'TaxAdjustmentRecord', 'TaxDatasetPreparationRecord', 'TaxAssessmentRecord']))
    ...
    database.exec("INSERT INTO InvoiceNumberSequenceOnboarding (ownerId, year, firstUnusedNumber, importedHighestNumber, importedCount, importedNumbersHash, confirmedBy) VALUES ('tenant-a', 2026, 42, 41, 2, 'hash', 'admin-a')")
    expect(() => database.exec("UPDATE InvoiceNumberSequenceOnboarding SET confirmedBy = 'admin-b' WHERE ownerId = 'tenant-a' AND year = 2026")).toThrow(/immutable/)
    expect(() => database.exec("DELETE FROM InvoiceNumberSequenceOnboarding WHERE ownerId = 'tenant-a' AND year = 2026")).toThrow(/immutable/)
