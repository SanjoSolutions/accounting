...
describe('document thumbnails', () => {
  ...
  it('keeps worker diagnostics separate from the binary thumbnail channel', async () => {
    const workerPath = join(process.cwd(), 'src', 'server', 'dataFixtures', 'noisyThumbnailWorker.mjs')

    await expect(generateDocumentThumbnail(minimalPdf(), { workerPath }))
      .resolves.toEqual(Buffer.from('RIFFxxxxWEBP'))
  })
