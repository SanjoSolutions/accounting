...
import { disposeFixedAsset, FixedAssetError } from '@/server/fixedAssetsRepository'
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json({ success: true, data: await disposeFixedAsset(user.id, user.actorId ?? user.id, (await params).id, input) })
