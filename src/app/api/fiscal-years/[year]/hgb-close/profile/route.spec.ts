...
describe('historical HGB profile API', () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.user.mockResolvedValue({ id: 'tenant-a' }); mocks.authorize.mockResolvedValue('tenant-a'); mocks.create.mockResolvedValue({ id: 'profile-1' }) })
  it('uses the authenticated tenant and route fiscal year', async () => {
    ...
    expect(response.status).toBe(201); expect(mocks.create).toHaveBeenCalledWith('tenant-a', 'tenant-a', 2025, expect.objectContaining({ reason: 'x' }))
  })
