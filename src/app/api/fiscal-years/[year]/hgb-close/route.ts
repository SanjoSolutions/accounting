...
export async function POST(request: Request, { params }: { params: Promise<{ year: string }> }) {
  ...
  try {
    ...
    return Response.json({ success: true, data: await evaluateAndPersistHgbClose(ownerId, user.actorId, await parseYear(params), body as Record<string, unknown>) }, { status: 201 })
  } catch (error) { return complianceError(error) }
}
