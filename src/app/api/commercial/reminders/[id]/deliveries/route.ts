...
import { deliverReceivablesReminder } from '@/server/receivablesReminderRepository'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const body = requireObject(await request.json()) as { requestKey: string; recipient: string; reason: string }
    const { id } = await context.params
    return Response.json({ success: true, data: await deliverReceivablesReminder(user.id, user.actorId, id, body.requestKey, body) }, { status: 201 })
