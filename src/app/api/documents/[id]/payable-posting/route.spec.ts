...
describe('incoming supplier invoice posting API', () => {
  ...
  it('posts only with the authenticated actor and explicit review choices', async () => {
    ...
    const input = { expenseAccountId: 'expense-a', dueDate: '2026-08-06', reason: 'Reviewed and approved', assessmentRateBasisPoints: 1900, supplyClassification: 'STANDARD_GOODS' }
  ...
  it('Given an EU-service treatment, when context is requested, then its distinct classification and fail-closed control readiness cross the authenticated API unchanged', async () => {
    const context = { posting: null, expenseAccounts: [], recommendedExpenseAccountId: null, recipientAssessedVatTreatment: { kind: 'DE_13B_EU_SERVICE', supportedAssessmentRatesBasisPoints: [1900], reason: 'UStG §3a(2), §13b(1); VAT Directive Article 196', configured: false } }
