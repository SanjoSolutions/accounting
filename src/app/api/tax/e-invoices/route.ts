...
import { configureInvoiceNumberSequence, issueStructuredInvoice, listStructuredInvoices, requireInvoiceIssuanceBody, type StructuredInvoiceInput } from '@/server/tax/structuredInvoices'
...
export async function POST(request: Request) {
  ...
  if (!user) return Response.json({ success: false }, { status: 401 })
  try {
    const body = requireInvoiceIssuanceBody(await request.json()) as StructuredInvoiceInput & { action?: string; requestKey?: string; year?: number; firstUnusedNumber?: number; confirmedExistingSeries?: boolean }
