...
export function TaxWorkspace({ year }: { year: number }) {
  ...
  return <div className="workspace py-4">
    ...
    {message && <p className="error-summary" role="status">{message}</p>}
    <div className="close-grid">
      <section className="panel"><h2>{t('applicability')}</h2>{applicability ? <><p>{t('deadline', { date: applicability.deadline })}</p><ul>{applicability.kinds.map(item => <li key={item}>{item}</li>)}</ul><p>{t('professionalReview')}</p></> : applicabilityUnavailable ? <p className="error-summary">{t('annualProfileRequired')}</p> : <p>{t('loading')}</p>}</section>
      <section className="panel"><h2>{t('prepare')}</h2><label>{t('kind')}<select className="form-select" value={kind} onChange={event => selectKind(event.target.value)}><option>USTVA</option>{applicability?.kinds.map(item => <option key={item}>{item}</option>)}</select></label><label>{t('period')}<input className="form-control" value={period} readOnly={kind !== 'USTVA'} onChange={event => datasetChanged(() => setPeriod(event.target.value))} /></label><label>{t('fields')}<textarea className="form-control" rows={5} value={fields} readOnly={Boolean(preparedDataset)} onChange={event => datasetChanged(() => setFields(event.target.value))} /></label><div className="form-check"><input id="tax-confirm" className="form-check-input" type="checkbox" disabled={!preparedDataset} checked={confirmed} onChange={event => setConfirmed(event.target.checked)} /><label className="form-check-label" htmlFor="tax-confirm">{t('confirm')}</label></div><div className="d-flex gap-2 mt-3"><button className="secondary-action" disabled={busy} onClick={() => void action('validate')}>{t('validate')}</button><button className="primary-action" disabled={busy || !confirmed || !preparedDataset} onClick={() => void action('submit')}>{t('submit')}</button></div></section>
    </div>
    <section className="panel mt-4"><h2>{t('history')}</h2>{workflows.length ? <div className="table-responsive"><table className="table"><thead><tr><th>{t('kind')}</th><th>{t('period')}</th><th>{t('state')}</th><th>{t('receipt')}</th><th>{t('action')}</th></tr></thead><tbody>{workflows.map(item => <tr key={item.submissionId}><td>{item.kind}</td><td>{item.period}</td><td>{item.state}</td><td>{item.receipt ?? '—'}</td><td>{['submitting', 'uncertain'].includes(item.state) ? <button className="secondary-action" disabled={busy} onClick={() => void recover(item)}>{t('recover')}</button> : '—'}</td></tr>)}</tbody></table></div> : <p>{t('empty')}</p>}</section>
  </div>
}
