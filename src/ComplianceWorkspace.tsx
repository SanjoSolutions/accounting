...
export function ComplianceWorkspace() {
  ...
  return <div className="workspace pb-4 compliance-workspace" aria-busy={busy || refreshing} data-compliance-ready={!busy && !refreshing ? 'true' : 'false'}>
    ...
    <section className="card panel"><div className="panel-title"><div><span className="step">4</span><h2>{t('workflows')}</h2></div><span className="badge text-bg-light hint">{t('workflowHint')}</span></div>
      ...
      {downloadableGobdPackages(overview?.reportingPackages ?? []).length > 0 && <div className="mt-4"><h3>GoBD audit and migration packages</h3><ul className="history-list">{downloadableGobdPackages(overview?.reportingPackages ?? []).map(item => <li key={item.id}><strong>{item.kind === 'AUDIT_EXPORT' ? 'Audit export' : 'Migration export'} v{item.version}</strong> · <code>{item.checksum}</code><br/><a className="btn btn-outline-secondary" href={`/api/compliance/packages/${encodeURIComponent(item.id)}`} download>Download verified GoBD package</a></li>)}</ul></div>}
...
export function downloadableGobdPackages<T extends { kind: string }>(packages: T[]) { return packages.filter(item => item.kind === 'AUDIT_EXPORT' || item.kind === 'MIGRATION_EXPORT') }
