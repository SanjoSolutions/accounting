...
export interface AuditExportSource {
  ...
  businessPartners?: readonly ExportRow[]; commercialDocuments?: readonly ExportRow[]
  paymentSettlements?: readonly ExportRow[]; settlementAllocations?: readonly ExportRow[]; correctionNettings?: readonly ExportRow[]
  bankAccounts?: readonly ExportRow[]; bankStatements?: readonly ExportRow[]; bankTransactions?: readonly ExportRow[]; bankTransactionMatches?: readonly ExportRow[]
  fixedAssets?: readonly ExportRow[]; assetEvents?: readonly ExportRow[]
}
...
const DATASETS = ['masterData', 'chartMappings', 'fiscalYears', 'journal', 'journalLines', 'openingClosing', 'vatDetails', 'evidence', 'auditEvents', 'taxSubmissions', 'openItems', 'businessPartners', 'commercialDocuments', 'paymentSettlements', 'settlementAllocations', 'correctionNettings', 'bankAccounts', 'bankStatements', 'bankTransactions', 'bankTransactionMatches', 'fixedAssets', 'assetEvents', 'cashBooks', 'cashBookEntries', 'cashDailyCloses'] as const
...
const PRIMARY_KEYS: Record<typeof DATASETS[number], readonly string[]> = { masterData: ['id'], chartMappings: ['accountId'], fiscalYears: ['id'], journal: ['id'], journalLines: ['id'], openingClosing: ['fiscalYearId', 'accountId'], vatDetails: ['id'], evidence: ['id'], auditEvents: ['id'], taxSubmissions: ['id'], openItems: ['id'], businessPartners: ['id'], commercialDocuments: ['id'], paymentSettlements: ['id'], settlementAllocations: ['id'], correctionNettings: ['id'], bankAccounts: ['id'], bankStatements: ['id'], bankTransactions: ['id'], bankTransactionMatches: ['id'], fixedAssets: ['id'], assetEvents: ['id'], cashBooks: ['id'], cashBookEntries: ['cashBookId', 'sequence'], cashDailyCloses: ['bookId', 'businessDate'] }
const SCHEMA_RELATIONSHIPS = [
  ...
  'commercialDocuments.businessPartnerId -> businessPartners.id', 'commercialDocuments.evidenceDocumentId -> evidence.id when supplied',
  'openItems.commercialDocumentId -> commercialDocuments.id', 'paymentSettlements.businessPartnerId -> businessPartners.id',
  'settlementAllocations.openItemId -> openItems.id', 'settlementAllocations.settlementId -> paymentSettlements.id',
  'correctionNettings.correctionDocumentId -> commercialDocuments.id', 'correctionNettings.originalOpenItemId -> openItems.id', 'correctionNettings.creditOpenItemId -> openItems.id',
  'bankStatements.bankAccountId -> bankAccounts.id', 'bankTransactions.bankAccountId -> bankAccounts.id', 'bankTransactions.statementId -> bankStatements.id',
  'bankTransactionMatches.bankTransactionId -> bankTransactions.id', 'bankTransactionMatches.openItemId -> openItems.id', 'bankTransactionMatches.settlementId -> paymentSettlements.id', 'bankTransactionMatches.allocationId -> settlementAllocations.id',
  'assetEvents.assetId -> fixedAssets.id',
...
export async function createAuditPackage(source: AuditExportSource, access: ExportAccess, sink: ExportAuditSink, authenticator?: MigrationPackageAuthenticator): Promise<AuditPackage> {
  ...
  for (const name of DATASETS) {
    ...
    if (!['masterData', 'chartMappings', 'fiscalYears', 'journal', 'journalLines', 'openingClosing', 'vatDetails', 'evidence', 'auditEvents', 'taxSubmissions'].includes(name) && supplied === undefined) { adapterRows[name] = []; continue }
...
function validateAccountingSchema(source: AuditExportSource) {
  ...
  for (const [dataset, rows] of [
    ['businessPartners', source.businessPartners], ['commercialDocuments', source.commercialDocuments], ['paymentSettlements', source.paymentSettlements],
    ['settlementAllocations', source.settlementAllocations], ['correctionNettings', source.correctionNettings], ['bankAccounts', source.bankAccounts], ['bankStatements', source.bankStatements],
    ['bankTransactions', source.bankTransactions], ['bankTransactionMatches', source.bankTransactionMatches], ['fixedAssets', source.fixedAssets], ['assetEvents', source.assetEvents],
  ] as const) for (const row of rows ?? []) requireText(row, 'id', dataset)
  ...
  requireUnique('masterData', source.masterData, row => String(row.id)); requireUnique('chartMappings', source.chartMappings, row => String(row.accountId)); requireUnique('fiscalYears', source.fiscalYears, row => String(row.id)); requireUnique('journal', source.journal, row => String(row.id)); requireUnique('journalLines', source.journalLines, row => String(row.id)); requireUnique('openingClosing', source.openingClosing, row => canonicalJson([row.fiscalYearId, row.accountId])); requireUnique('vatDetails', source.vatDetails, row => String(row.id)); requireUnique('evidence', source.evidence, row => String(row.id)); requireUnique('auditEvents', source.auditEvents, row => String(row.id)); requireUnique('taxSubmissions', source.taxSubmissions, row => String(row.id)); requireUnique('openItems', source.openItems ?? [], row => String(row.id));
  for (const [dataset, rows] of [
    ['businessPartners', source.businessPartners], ['commercialDocuments', source.commercialDocuments], ['paymentSettlements', source.paymentSettlements],
    ['settlementAllocations', source.settlementAllocations], ['correctionNettings', source.correctionNettings], ['bankAccounts', source.bankAccounts], ['bankStatements', source.bankStatements],
    ['bankTransactions', source.bankTransactions], ['bankTransactionMatches', source.bankTransactionMatches], ['fixedAssets', source.fixedAssets], ['assetEvents', source.assetEvents],
  ] as const) requireUnique(dataset, rows ?? [], row => String(row.id))
  requireUnique('cashBooks', source.cashBooks ?? [], row => String(row.id)); requireUnique('cashBookEntries', source.cashBookEntries ?? [], row => canonicalJson([row.cashBookId, row.sequence])); requireUnique('cashDailyCloses', source.cashDailyCloses ?? [], row => canonicalJson([row.bookId, row.businessDate])); requireUnique('cashDailyCloses id', source.cashDailyCloses ?? [], row => String(row.id))
  ...
  const partnerIds = new Set((source.businessPartners ?? []).map(row => row.id)); const commercialById = new Map((source.commercialDocuments ?? []).map(row => [row.id, row])); const commercialIds = new Set(commercialById.keys()); const openItemById = new Map((source.openItems ?? []).map(row => [row.id, row])); const openItemIds = new Set(openItemById.keys()); const settlementIds = new Set((source.paymentSettlements ?? []).map(row => row.id)); const allocationIds = new Set((source.settlementAllocations ?? []).map(row => row.id)); const bankAccountIds = new Set((source.bankAccounts ?? []).map(row => row.id)); const bankStatementIds = new Set((source.bankStatements ?? []).map(row => row.id)); const bankTransactionIds = new Set((source.bankTransactions ?? []).map(row => row.id)); const fixedAssetIds = new Set((source.fixedAssets ?? []).map(row => row.id))
  if ((source.commercialDocuments ?? []).some(row => !partnerIds.has(row.businessPartnerId) || (row.postingJournalEntryIncluded === true && !journals.has(row.postingJournalEntryId)) || (row.evidenceDocumentId !== null && row.evidenceDocumentId !== undefined && !new Set(source.evidence.map(item => item.id)).has(row.evidenceDocumentId)))) throw new Error('Commercial-document export relationships are invalid.')
  if ((source.commercialDocuments?.length ?? 0) > 0 && (source.openItems ?? []).some(row => !commercialIds.has(row.commercialDocumentId))) throw new Error('Open-item export relationships are invalid.')
  if ((source.paymentSettlements ?? []).some(row => !partnerIds.has(row.businessPartnerId) || (row.journalEntryIncluded === true && !journals.has(row.journalEntryId)))) throw new Error('Payment-settlement export relationships are invalid.')
  if ((source.settlementAllocations ?? []).some(row => !openItemIds.has(row.openItemId) || !settlementIds.has(row.settlementId) || (row.journalEntryIncluded === true && !journals.has(row.journalEntryId)))) throw new Error('Settlement-allocation export relationships are invalid.')
  if ((source.correctionNettings ?? []).some(row => { const correction = commercialById.get(row.correctionDocumentId); const original = openItemById.get(row.originalOpenItemId); const credit = openItemById.get(row.creditOpenItemId); return !correction || !original || !credit || credit.commercialDocumentId !== correction.id || correction.correctsId !== original.commercialDocumentId || (row.journalEntryIncluded === true && !journals.has(row.journalEntryId)) })) throw new Error('Correction-netting export relationships are invalid.')
  if ((source.bankStatements ?? []).some(row => !bankAccountIds.has(row.bankAccountId)) || (source.bankTransactions ?? []).some(row => !bankAccountIds.has(row.bankAccountId) || !bankStatementIds.has(row.statementId)) || (source.bankTransactionMatches ?? []).some(row => !bankTransactionIds.has(row.bankTransactionId) || !openItemIds.has(row.openItemId) || !settlementIds.has(row.settlementId) || !allocationIds.has(row.allocationId) || !journals.has(row.journalEntryId))) throw new Error('Bank-provenance export relationships are invalid.')
  if ((source.assetEvents ?? []).some(row => !fixedAssetIds.has(row.assetId))) throw new Error('Fixed-asset export relationships are invalid.')
