...
export interface XRechnungSellerContact { name: string; telephone: string; email: string }
export type XRechnungElectronicAddressScheme = '0204' | '9930' | 'EM'
export interface XRechnungElectronicAddress { schemeId: XRechnungElectronicAddressScheme; value: string }
export interface XRechnungInvoiceFields {
  buyerReference: string
  buyerElectronicAddress: XRechnungElectronicAddress
  sellerContact: XRechnungSellerContact
}
...
export function generateUblInvoice(data: Omit<StructuredInvoiceData, 'syntax'>): Uint8Array {
  return generateUblInvoiceWithProfile(data)
}

export function generateXRechnungUblInvoice(data: Omit<StructuredInvoiceData, 'syntax'> & XRechnungInvoiceFields): Uint8Array {
  const fields = [data.buyerReference, data.buyerElectronicAddress?.value, data.sellerContact.name, data.sellerContact.telephone, data.sellerContact.email, data.seller.vatId, data.paymentTerms, data.paymentIban]
  if (fields.some(value => !value?.trim())) throw new EInvoiceValidationError(['XRechnung requires BuyerReference, buyer electronic address, seller contact and electronic address, payment terms and IBAN.'])
  if (!['0204', '9930', 'EM'].includes(data.buyerElectronicAddress?.schemeId)) throw new EInvoiceValidationError(['The buyer electronic-address scheme is not supported.'])
  if (data.buyerElectronicAddress.schemeId === '0204' && !isValidLeitwegId(data.buyerElectronicAddress.value)) throw new EInvoiceValidationError(['A 0204 buyer electronic address must be a valid Leitweg-ID with a Modulo-97 checksum.'])
  if (data.buyerElectronicAddress.schemeId === '9930' && !isVatIdForCountry(data.buyerElectronicAddress.value, 'DE')) throw new EInvoiceValidationError(['A 9930 buyer electronic address must be a valid German VAT ID.'])
  if (data.buyerElectronicAddress.schemeId === 'EM' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.buyerElectronicAddress.value)) throw new EInvoiceValidationError(['An EM buyer electronic address must be an email address.'])
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.sellerContact.email)) throw new EInvoiceValidationError(['XRechnung seller contact email is invalid.'])
  return generateUblInvoiceWithProfile(data, data)
}

export function isValidLeitwegId(value: string) {
  const match = /^(\d{2}|\d{3}|\d{5}|\d{8}|\d{9}|\d{12})(?:-([A-Z0-9]{1,30}))?-(\d{2})$/i.exec(value.trim())
  if (!match) return false
  const comparable = `${match[1]}${match[2] ?? ''}${match[3]}`.toUpperCase().replace(/[A-Z]/g, character => String(character.charCodeAt(0) - 55))
  let remainder = 0
  for (const digit of comparable) remainder = (remainder * 10 + Number(digit)) % 97
  return remainder === 1
}

function generateUblInvoiceWithProfile(data: Omit<StructuredInvoiceData, 'syntax'>, xrechnung?: XRechnungInvoiceFields): Uint8Array {
  ...
  const customizationId = xrechnung ? 'urn:cen.eu:en16931:2017#compliant#urn:xeinkauf.de:kosit:xrechnung_3.0' : 'urn:sanjo-solutions:accounting:ubl:1'
  const profileId = xrechnung ? 'urn:fdc:peppol.eu:2017:poacc:billing:01:1.0' : 'urn:sanjo-solutions:accounting:billing:1'
...
function partyXml(role: string, party: EInvoiceParty, xrechnung?: { contact?: XRechnungSellerContact; endpoint: XRechnungElectronicAddress }) { const registrations = `${party.vatId ? `<cac:PartyTaxScheme><cbc:CompanyID>${esc(party.vatId)}</cbc:CompanyID><cac:TaxScheme><cbc:ID>VAT</cbc:ID></cac:TaxScheme></cac:PartyTaxScheme>` : ''}${party.taxId ? `<cac:PartyTaxScheme><cbc:CompanyID>${esc(party.taxId)}</cbc:CompanyID><cac:TaxScheme><cbc:ID>FC</cbc:ID></cac:TaxScheme></cac:PartyTaxScheme>` : ''}`; const endpoint = xrechnung ? `<cbc:EndpointID schemeID="${esc(xrechnung.endpoint.schemeId)}">${esc(xrechnung.endpoint.value)}</cbc:EndpointID>` : ''; const contact = xrechnung?.contact ? `<cac:Contact><cbc:Name>${esc(xrechnung.contact.name)}</cbc:Name><cbc:Telephone>${esc(xrechnung.contact.telephone)}</cbc:Telephone><cbc:ElectronicMail>${esc(xrechnung.contact.email)}</cbc:ElectronicMail></cac:Contact>` : ''; return `<cac:${role}><cac:Party>${endpoint}<cac:PartyName><cbc:Name>${esc(party.name)}</cbc:Name></cac:PartyName><cac:PostalAddress><cbc:StreetName>${esc(party.street)}</cbc:StreetName><cbc:CityName>${esc(party.city)}</cbc:CityName><cbc:PostalZone>${esc(party.postalCode)}</cbc:PostalZone><cac:Country><cbc:IdentificationCode>${esc(party.countryCode)}</cbc:IdentificationCode></cac:Country></cac:PostalAddress>${registrations}<cac:PartyLegalEntity><cbc:RegistrationName>${esc(party.name)}</cbc:RegistrationName></cac:PartyLegalEntity>${contact}</cac:Party></cac:${role}>` }
