...
export function originalVatPostingMarkers(posting: VatPostingDetail) {
  if (!trustedVatPostings.has(posting) || posting.reversalOf) throw new VatValidationError(['Durable original markers require an exact newly calculated original VAT posting.'])
  return Object.freeze([`bound-source:${JSON.stringify(posting.sourceId)}`, `original:${JSON.stringify(posting.sourceId)}:${postingIdentity(posting)}`])
}
...
function calculateVatInternal(split: VatSourceSplit, rules: VatRuleBook, original?: VatPostingDetail, reversalRegistry?: VatReversalRegistry, pendingReversals?: { sourceId: string; identity: string }[], pendingOriginals?: VatPostingDetail[]): VatPostingDetail {
  ...
  if (rule.case === 'intra-eu' && split.supplyKind !== 'goods') throw new VatValidationError(['Intra-EU acquisition VAT requires an explicit goods supply classification.'])
...
export const representativeGermanVatRules: readonly VatRule[] = Object.freeze([
  ...
  { id: 'EU_13B_SERVICE_RECIPIENT', version: 1, validFrom: '2010-01-01', jurisdiction: 'DE/EU', case: 'reverse-charge', reverseChargeRole: 'recipient', rateBasisPoints: 1900, deductibleShareBasisPoints: 10_000, outputAccount: '1787', inputAccount: '1577', returnBoxes: [{ box: '46', value: 'net-base', direction: 'purchase' }, { box: '47', value: 'output-tax', direction: 'purchase' }, { box: '67', value: 'input-tax', direction: 'purchase' }], reason: 'UStG §3a(2), §13b(1); VAT Directive Article 196' },
  ...
  { id: 'EU_ACQUISITION', version: 1, validFrom: '2007-01-01', jurisdiction: 'DE/EU', case: 'intra-eu', rateBasisPoints: 1900, deductibleShareBasisPoints: 10_000, outputAccount: '1774', inputAccount: '1574', returnBoxes: [{ box: '89', value: 'net-base', direction: 'purchase' }, { box: '61', value: 'input-tax', direction: 'purchase' }], reason: 'UStG §1a, §13(1)6, §15(1)3' },
