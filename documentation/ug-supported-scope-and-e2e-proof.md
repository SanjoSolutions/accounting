...
validates a freshly generated invoice, and retains both XML and HTML reports.
...
| UG-XRECHNUNG-CONTRACT-01 | Generated profiles pass the official KoSIT validator with retained validation reports. | `xrechnung.contract.spec.ts` passes KoSIT validator 1.6.2 with XRechnung 3.0.2 rules dated 2026-01-31 for public 0204 Leitweg-ID, business 9930 VAT-ID mixed 7%/19%, and EM email endpoint invoices; the `xrechnung-contract` CI job retains every official XML/HTML report |
...
| UG-ELSTER-CONTRACT-01 | Opt-in official qualified test submission for every enabled form/version, archived redacted protocol. | `qualified-tax-gateway.contract.spec.ts` and its environment-approved manual workflow fail closed unless all eight enabled 2025/2026 form versions validate, reject an invalid dataset and return accepted TEST/STAGING submission evidence; execution remains an external prerequisite and a skip is explicitly not evidence |
| UG-DISCLOSURE-CONTRACT-01 | Opt-in official Unternehmensregister Webservice validation and non-binding acceptance for micro-entity Hinterlegung and small-entity Offenlegung, with archived redacted protocols. | `qualified-disclosure-gateway.contract.spec.ts`, BDD configuration/protocol tests and the protected manual workflow require the provider-issued schema, electronically identified submitter, exact qualification identity, invalid-package rejection and retained TEST/STAGING receipt for both supported cases; provider access remains an external prerequisite and a skip is explicitly not evidence |
